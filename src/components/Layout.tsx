...
const Layout = () => {
  ...
  // Page transition effect with even more minimal wave
  useEffect(() => {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background">
      <main className="flex-1 container px-4 pb-20 pt-6 max-w-5xl mx-auto">
        <div
          className={`transition-all duration-120 ease-in-out transform ${
            isPageTransitioning ? "opacity-0 translate-y-0.5" : "opacity-100 translate-y-0"
          }`}
        >
          <Outlet />
        </div>
